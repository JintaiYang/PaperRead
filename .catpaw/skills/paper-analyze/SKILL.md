...
> **痛点**：手动阅读论文并整理笔记耗时 2-4 小时/篇，且不同论文笔记结构不统一、图片引用遗漏频繁、实验数据摘录不完整、知识图谱与笔记脱节。
> **用户画像**：搜推/AI 方向研究员、研究生，每周阅读 3-5 篇论文，使用 Obsidian 管理论文知识库。
...
### 步骤4：执行深度分析 `[freedom: low]`

按以下维度系统分析论文，**每个维度必须做到深入而非概括性描述**：

1. **摘要分析**：提取关键概念、研究目标、主要贡献，生成中文翻译
2. **方法论分析**：
   - 每个模块/子模块必须包含**完整的数学公式推导**（逐行写出，不可跳步）
   - 公式之后必须用**自然语言逐符号解释**含义
   - 必须配**直觉性解释或例子**帮助理解（如用具体业务场景说明为什么这样设计）
   - 模块间的数据流和依赖关系必须显式描述
3. **实验分析**：
   - 主实验结果表必须列出论文中**所有基线方法的完整数据**，不可只摘录最佳基线
   - 每组实验数据必须配**逐条分析**（而非只总结结论）
   - 消融实验需逐项列出变体及其量化影响
4. **洞察生成**：研究价值、局限性、未来工作、与相关工作对比

分析深度与论文本身的信息密度匹配——论文提供了多少细节，笔记就要覆盖多少细节。不可为了简洁而省略论文中的关键信息。
...
3. 插入论文图片引用（使用 Obsidian wikilink 语法 `![[filename.png|800]]`）
   - **必须引用 `images/` 目录中的所有 PNG 图片**，每张图配 caption 说明
   - 在笔记对应的分析段落中就近插入图片，而非集中放在末尾
